...
export class AuthService {
  ...
  private async generateHash(password: string): Promise<string> {
    const salt = await bcrypt.genSalt(this.authConfig.hashSaltRounds || DEFAULT_HASH_SALT_ROUNDS)
  ...
  findByProvider(identifier: string, provider: string): Promise<AuthMetadata | undefined> {
    return this.authMetadataRepository.findOne(q =>
      q.whereEqualTo('identifier', identifier).whereEqualTo('provider', provider),
    )
  }

  async findByAuthCode(authCode: string, provider: string): Promise<AuthMetadata | undefined> {
  ...
  async createUser(input: UserMetadata, credentials?: Credentials): Promise<AuthInfo> {
    ...
    await this.authMetadataRepository.save({
      ...
      passwordHash: credentials?.password ? await this.generateHash(credentials.password) : null,
  ...
  async issueAuthCode(identifier: string, provider: string) {
    ...
    if (!authMetadata) {
      throw new NotFoundException('Authentication failed: Auth metadata not found')
    }
    const code = generateNumericId()
    await this.authMetadataRepository.update({
      id: authMetadata.id,
      authCode: code,
      authCodeExpiresAt: new Date(calculateExpiresAt(this.authConfig.authCodeExpiry ?? '5m')),
    })
    return code
  }
